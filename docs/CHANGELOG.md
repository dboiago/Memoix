## Memoix - v2.1.0+12 - 2026-06-02

### Added
- Walk-in omnibar: SQL-based retrieval path for users without an agent key, routing through structured filters derived from natural language queries
- Walk-in omnibar: BYOK generation pass producing dry, observational reason lines over SQL results for users with an agent key
- Walk-in omnibar: vibe fields (`preferredCourses`, `prefersLongProject`, `excludeCuisine`) wired through classifier, retrieval service, and RPC
- Walk-in omnibar: "Show me anyway" fallback for unmappable queries without an agent key, surfacing discovery results under neutral framing
- Walk-in omnibar: multi-cuisine filtering; classifier forwards full cuisine list rather than first match
- Corpus: `onCreate` handler now creates all five FTS virtual tables on fresh installs; previously only ran on upgrade paths

### Changed
- Walk-in omnibar: SQL result scoring rebalanced — rating signal removed, `isFavourite` and `cookCount` now read from both recipe and entry payload shapes to correctly score cellar and cheese corpus entries, query name match adds score boost, favourite contribution suppressed when untried filter is active
- Walk-in omnibar: candidate pool rebuilt only on provider data or intent change; selection filters run each build against cached pool
- Corpus: Supabase transmission client retries client acquisition up to three times on initialisation race before silently aborting; backfill no longer silently drops on cold launch

### Fixed
- Fresh install missing all five FTS virtual tables (`recipes_fts`, `pizzas_fts`, `sandwiches_fts`, `cellar_fts`, `cheese_fts`); recipe save failed with `no such table` on clean installs from the store
- Walk-in omnibar: all eight `sqlFilter` RPC parameter keys were using wrong naming convention and had never applied structured filters
- Walk-in omnibar: `hasActiveProvider && isUnmappable` fell through to vector query path instead of routing to `sqlDiscover`
